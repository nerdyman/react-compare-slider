...
export interface ReactCompareSliderProps
    extends Partial<ReactCompareSliderCommonProps> {
...
> = ({
    ...
    const updateInternalPosition = useCallback(
        function updateInternalCall({
            ...
            const zoomScale = _portrait
                ? height /
                  ((rootContainerRef.current as HTMLDivElement).offsetHeight ||
                      1)
                : width /
                  ((rootContainerRef.current as HTMLDivElement).offsetWidth ||
                      1);
            ...
            const nextInternalPositionPc =
                (adjustedPosition /
                    (_portrait ? adjustedHeight : adjustedWidth)) *
                100;
    ...
    const handleResize: (
        resizeProps: UseResizeObserverHandlerParams,
    ) => void = useCallback(
        ({ width, height }) => {
            const {
                width: scaledWidth,
            ...
            updateInternalPosition({
                ...
                boundsPadding,
                x:
                    ((width / 100) * internalPositionPc.current * scaledWidth) /
                    width,
                y:
                    ((height / 100) *
                        internalPositionPc.current *
