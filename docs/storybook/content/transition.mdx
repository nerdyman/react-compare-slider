...
import { Transition } from './stories/00-demos/00-index.stories'
...
transition only applies to `pointerdown` events to ensure the user doesn't have to wait for the transition to
complete if they are dragging the slider. This means the transition will be cancelled if `pointerdown` is
followed by a `pointermove` event.
