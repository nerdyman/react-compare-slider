...
const Template: Story<ReactCompareSliderDetailedProps> = (args) => <ReactCompareSlider {...args} />;
...
PointerMovement.play = async ({ canvasElement }) => {
  const canvas = within(canvasElement);
  const rootComponent = canvas.queryByTestId(PointerMovement.args['data-testid']) as Element;
  ...
  await waitFor(() => expect(PointerMovement.args.onPositionChange).toHaveBeenCalledWith(75));
  ...
  await waitFor(() => expect(PointerMovement.args.onPositionChange).toHaveBeenCalledWith(100));
  ...
  await waitFor(() => expect(PointerMovement.args.onPositionChange).toHaveBeenCalledWith(0));
};
...
HandleInteractions.play = async ({ canvasElement }) => {
  const canvas = within(canvasElement);
  const rootComponent = canvas.queryByTestId(HandleInteractions.args['data-testid']) as Element;
