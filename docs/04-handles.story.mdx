import { Canvas, ArgsTable, Meta, Story } from '@storybook/addon-docs/blocks';

import { ReactCompareSliderHandle } from 'react-compare-slider';

<Meta title="Docs/Handles" component={ReactCompareSliderHandle} />

# Using Handles

- [ReactCompareSliderHandle](#reactcomparesliderhandle)
- [Custom Default Handle Usage](<#custom-default-handle-usage>)
- [Custom Standalone Handle Usage](<#custom-standalone-handle-usage>)
- [Live Examples](<#live-examples>)

```js codesandbox=react-compare-slider-simple-example-9si6l?file=/src/index.js
import React from "react";
import ReactDOM from "react-dom";
import "./styles.css";

import { CustomHandle } from "./App";

const rootElement = document.getElementById("root");
ReactDOM.render(<CustomHandle style={{ height: '100vh' }} />, rootElement);

```

Custom Handles can be used via the `handle` prop on the main slider component.
If a custom `handle` is not supplied `ReactCompareSliderHandle` will
be used instead.

<br />

### `ReactCompareSliderHandle`

`ReactCompareSliderHandle` supports the `portrait` prop to automatically flip
itself to match the desired orientation along with individual props to style the lines and button. 
It also provides class names and accepts any valid `div`
component props so you can easily customize it to fit your requirements.

- `__rcs-handle-root`    - The root element.
- `__rcs-handle-button`  - The circular element shown in the middle.
- `__rcs-handle-line`    -  For each line either side of the button.

<br />

#### Props

<ArgsTable of={ReactCompareSliderHandle} />

### Custom `ReactCompareSliderHandle` usage

The colors used in `ReactCompareSliderHandle` are inherited from the root element's `color`
property using the `currentColor` keyword (except for `boxShadow`). 
To set all colors in sync just change the `style` property or use the `__rcs-handle-root` class.

<Canvas>
  <Story id="demos-handles--inherited-color" />
</Canvas>

### Custom Standalone Handle Usage

```jsx
import { ReactCompareSlider } from 'react-compare-slider';

/** Render a completely custom handle. */
const MyCustomHandle = () => (
  <div style={{ width: '5px', height: '100%', backgroundColor: red }}></div>;
)

<ReactCompareSlider handle={<MyCustomHandle />} />
```

### Live Examples

Check out the [handles demos](/?path=/story/demos-handles--blurred-arrows) to experiment with handles.
